...
class CircularProgressBarUtils {
    ...
    static void checkPositiveOrZero(float number, String name) {
        if (number < 0) {
            throw new IllegalArgumentException(
                    String.format(Locale.CHINA, "%s %f must be positive", name, number));
        }
    }
    ...
    static float getAnimatedFraction(ValueAnimator animator) {
        float fraction = animator.getDuration() > 0 ? ((float) animator
                .getCurrentPlayTime()) / animator.getDuration() : 0f;
